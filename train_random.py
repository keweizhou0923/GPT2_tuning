...
parser.add_argument('--start_len', metavar='N', type=int, default=15, help='test start with N words')
...
#         print(test_sample)
        ...
        def split_test(x):
            tmp = (" ".join(x.split()[:args.start_len]))
            return(np.stack(enc.encode(tmp)))
         
        def generate_samples():
            test_sampleset_s = random.sample(test_sampleset, args.sample_num)
            test_start_set =[  split_test(ele) for ele in test_sampleset_s]  
            all_text = []
            index = 0
            while index < args.sample_num:
                context_tokens = test_start_set[index]
                out = sess.run(
                    tf_sample,
...
#             print("start sentense :" + enc.decode(token_test ))
            ...
            with open(
                    os.path.join(SAMPLE_DIR, args.run_name,
                                 'samples_ori-{}').format(counter), 'w') as fp:
                fp.write('\n'.join(test_sampleset))            

        def sample_batch():
            return [data_sampler.sample(1024) for _ in range(args.batch_size)]
            
        test_sampleset = test_sampleset_gen(args.sampledataset)
